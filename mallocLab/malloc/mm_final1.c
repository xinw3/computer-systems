...
static const word_t alloc_mask = 1;
static const word_t free_alloc_mask = 2;
...
static word_t pack(size_t size, bool prev, bool alloc);

static size_t extract_size(word_t header);
static size_t get_size(block_t *block);
static word_t get_payload_size(block_t *block);

static bool extract_alloc(word_t header, bool prev);
static bool get_alloc(block_t *block, bool prev);

static void 
write_header(block_t *block, size_t size, bool prev, bool alloc);
static void 
...
void free(void *bp)
{
    ...
    bool prev_status = extract_alloc(block->header, true);
    write_header(block, size, prev_status, false);
...
static block_t *coalesce(block_t * block) 
{
    ...
    bool prev_alloc = get_alloc(block, true);
    bool next_alloc = get_alloc(block_next, false);
    size_t size = get_size(block);
 ...
 */
static bool extract_alloc(word_t word, bool prev)
{
    if (!prev)
    {
...
static bool get_alloc(block_t *block, bool prev)
{
    return extract_alloc(block->header, prev);
}
...
{ 
    ...
    if (get_size(heap_listp) != wsize || !get_alloc(heap_listp, false))
    {
    ...
    /* Check epilogue */
    if (bp->header != 0 || !get_alloc(bp, false))
    {
